...
class BiquadFilter:
    ...
    def add_option_type(self):
        self.inc_opt_counter()
        self.type_options = ["LPF","HPF","Bandpass","Notch"]
        self.selected_type_option = StringVar()
        self.selected_type_option.set(self.type_options[0])  # Set the default option
        # Create the dropdown menu
        
        self.type_options_dropdown = OptionMenu(self.canvas.master, self.selected_type_option, *self.type_options)
        filterbank_step_opt_dropdown = self.canvas.create_window(self.x + self.w/2,  self.y + self.opt_start_y+self.opt_height*self.opt_counter, window=self.type_options_dropdown, anchor="center", tags=("block", self.tag))
    ...
    def add_option_freq(self):
        self.inc_opt_counter()
        self.freq_entry = tk.Entry(self.canvas, width=5)
        self.freq_entry.insert(0, "1")  # Default value
        
        # Embed the entry widget into the canvas and tag it
        freq_entry_text   = self.canvas.create_text(self.x+0.2*self.w, self.y+self.opt_start_y+self.opt_height*self.opt_counter, text="Freq\n[Hz]", font=("Arial", 14), tags=("block", self.tag))
        self.freq_window  = self.canvas.create_window(self.x + 0.7*self.w, self.y + self.opt_start_y+self.opt_height*self.opt_counter, window=self.freq_entry, tags=("input_freq_entry", self.tag), anchor="center")
        
        self.freq_entry.bind("<FocusOut>", self.freq_limits_check)
    ...
    def freq_limits_check(self, event):
        ...
        try:
            ...
            self.freq_entry.insert(0, "1")  # Reset to default if invalid

    def validate_input(self, event):
        value = self.ch_entry.get()
        try:
            num = int(value)
            if num < 1 or num > 22000:
                raise ValueError("Out of bounds")
        except ValueError:
            self.ch_entry.delete(0, tk.END)
            self.ch_entry.insert(0, "1")  # Reset to default if invalid
...
class MovingAverage:
    ...
    def add_option_buffer(self):
        self.inc_opt_counter()
        self.bufer_entry = tk.Entry(self.canvas, width=5)
        self.bufer_entry.insert(0, "1")  # Default value
        
        # Embed the entry widget into the canvas and tag it
        buffer_entry_text   = self.canvas.create_text(self.x+0.2*self.w, self.y+self.opt_start_y+self.opt_height*self.opt_counter, text="Buff\nsize", font=("Arial", 14), tags=("block", self.tag))
        self.freq_window  = self.canvas.create_window(self.x + 0.7*self.w, self.y + self.opt_start_y+self.opt_height*self.opt_counter, window=self.bufer_entry, tags=("input_buffer_entry", self.tag), anchor="center")
        
        self.bufer_entry.bind("<FocusOut>", self.buff_limits_check)
    ...
    def buff_limits_check(self, event):
        value = self.bufer_entry.get()
        try:
            num = int(value)
            if num < 1 or num > 22000:
                raise ValueError("Out of bounds")
        except ValueError:
            self.bufer_entry.delete(0, tk.END)
            self.bufer_entry.insert(0, "1")  # Reset to default if invalid
#   ------------------------------------------------------
